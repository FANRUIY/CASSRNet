...
#from models.archs.RCAN_arch import DSConv
...
class classSR_3class_rcan(nn.Module):
    def __init__(self, in_nc=3, out_nc=3):
        ...
        self.net1 = RCAN(n_resgroups=10, n_resblocks=20, n_feats=36, res_scale=1, 
                         scale=4)
        self.net2 = RCAN(n_resgroups=10, n_resblocks=20, n_feats=48, res_scale=1, 
                         scale=4)
        self.net3 = RCAN(n_resgroups=10, n_resblocks=20, n_feats=64, res_scale=1, 
                         scale=4)
        
        #self.net1 = RCAN(n_resgroups=10, n_resblocks=20, n_feats=36, res_scale=1, n_colors=3, rgb_range=1,  reduction=16,
         #                scale=4)
        #self.net2 = RCAN(n_resgroups=10, n_resblocks=20, n_feats=50, res_scale=1, n_colors=3, rgb_range=1,  reduction=16,
        #                 scale=4)
        #self.net3 = RCAN(n_resgroups=10, n_resblocks=20, n_feats=64, res_scale=1, n_colors=3, rgb_range=1,  reduction=16, 
           #              scale=4)
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.lastOut = nn.Linear(32, 3) # 这里期待输入特征是32维，输出3类
    ...
    def forward(self, x):
      out = self.CondNet(x)  # [B, 32, H_out, W_out]
      out = nn.AdaptiveAvgPool2d(1)(out)  # 固定输出 [B, 32, 1, 1]
      out = out.view(out.size(0), -1)  # [B, 32]
      out = self.lastOut(out)  # 线性层输入维度匹配
      return out
