...
def cal_FLOPs(which_model,num_ress):
    if which_model =='classSR_3class_fsrcnn_net':
        ...
        flops = (9.13 * num_ress[0] + 12.15 * num_ress[1] + 16.86 * num_ress[2]) / sum(num_ress)
        percent = flops / 16.86
    elif which_model == 'classSR_3SRNet':
    # 使用估算的 FLOPs 值（单位：GMac）
      flops_CARN = 1.16
      flops_MSRResNet = 5.19
      flops_RCAN = 32.63
      flops = (flops_CARN * num_ress[0] + flops_MSRResNet * num_ress[1] + flops_RCAN * num_ress[2]) / sum(num_ress)
      percent = flops / flops_RCAN
